...
The transition temperature can be solved by setting an equation of $abs(eigenvalue)==1$ because 
\begin{enumerate}
\item the eigenvalues are complex numbers, and the abs(eigenvalue) is the 1st norm; 
\item as what I read from mathematical literatures, for complex eigenvalues, if the 1st norm is smaller than 1, the recursive equations can reach a stable fixed point; otherwise, it won't. Also, complex eigenvalues usually mean oscillating flow of the parameters;
\item The actually equation to get the transition is very long, complex, and not solvable analytically. We won't show it here, but we can find the transition for HNNP $\sim$ HN6 for different $y$ values. (An example of the eigenvalues are shown in Fig \ref{fig:hnnp_eigs}.)
\end{enumerate}
...
\protect\caption{The absolute values (1st norm) of the 2 complex eigenvalues for HNNP (y=0). For $0<\mu<1$, that is for the ferromagnetic case; while the antiferromagnetic region is $\mu>1$. You see the transition temperature is $\mu=3.0$ because after that the $|eigenvalues|>1$ . }
\label{fig:hnnp_eigs} 
\end{figure}
...
\protect\caption{$\kappa$ vs $1/\mu$ calculating using Eq.\ref{eq:fps_k}. $y=0$ corresponds to HNNP; $y=1$ corresponds to HN6; and $y<0$ corresponds that some bonds are ferromagnetic;. In AF model, our temperature parameter $\mu$ is greater than 1. In this case, $1/\mu = 0 \Leftrightarrow T = 0$; $1/\mu = 1 \Leftrightarrow T\rightarrow \infty $. }
\label{fig:HN35_ys} 
\end{figure*}
