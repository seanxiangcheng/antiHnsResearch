...
int L=0, K=0, Flat=80, Seed, Name = 0, Total_Neig_Links = 0; 	// Input parameters
float Flatness=0.8;
int StatesLength = 0, MaxLength, MaxState, CurrentState, CurrentIndex, MinState;
double Logfnow=1.0, Logfmin=0.0000001, MinStateLogg, MaxSteps=2.0e13;
...
// print neighbors loc
int Print_Neighbors(int neighbors[], int neighloc[])
{
    int i=0, j=0;
    printf("neighbors of L=%d:\n", L);
    for(i=0; i<L; i++)
    {
...
void Initialize(struct StateInfo state[], int site[], int neighbors[], int neighloc[]){
    ...
    Total_Neig_Links = neig_index;
    printf("\n\n Total neig links: %d \n\n", neig_index);
    state[0].E = neig_index/2;
    state[0].logg = 0.0;
    state[0].H = 0.0;
...
int main(int argc, char *argv[]){
    ...
    char config_file[20]="H6AF_Config";
  	char buf[8];
    ...
    for(j = 0; j <100; j++) {
  	...
  	printf("\n\n L=%d; Flatness=%f;\n\n", L, Flatness);
  	MaxLength = (6*L); // this is just for HN3
  	int site[L];
    ...
    Initialize(state, site, neighbors, neighloc);
  	Total_Steps=0.0;
    //Print_Neighbors(neighbors, neighloc);
  	sprintf(buf,"%d", K);
    strcat(dos_file, buf);
    ...
    fprintf(fp, " Total links: %d\n Bad Bonds: %d\n\n", Total_Neig_Links, (Total_Neig_Links/2+MinState)/2);
    fprintf(fp, "*********** HNNP, L=%d, Lowest Energy=%d, configurations *************\n", L, MinState);
