...
/* Check bad bonds number */
int Check_Bad_Bonds(int sites[], int neighbors[], int neighloc[])
{
  int i, j, neig_sum = 0, energy = 0, bad_bond_num=0;
  for(i = 0; i < L; i++)
  {
      neig_sum = 0;
      for(j = neighloc[i]; j < neighloc[i+1]; j++ ){
      ...
      energy = sites[i] * neig_sum;
      bad_bond_num = bad_bond_num + (neighloc[i+1]-neighloc[i]+ energy)/2;
  }
  
  return(bad_bond_num/2);
}

// Check whether it is a new GS, if so store it
int Fit_Config(int site[], short configs[], short *config_num, int bad_bonds_num[], int neighbors[], int neighloc[])
{
    ...
    if(*config_num==0 || diff_num == *config_num)
    {
        for(i=0; i< L; i++)
        {
            configs[(*config_num)*L + i] = site[i];
        }
        bad_bonds_num[*config_num] = Check_Bad_Bonds(site, neighbors, neighloc);
        (*config_num)++;
...
// Check whether the configration is right
int Check_Configs_Wrong(short configs[], short config_num)
{
    ...
    for(ci=0; ci<config_num-1; ci++)
    {
        for(cj=ci+1; cj<config_num; cj++)
        {
            for(i=0; i<L; i++)
            {
                if(configs[ci*L+i] != configs[cj*L+i])
                {
                  flag = 1;
                  break;
                }
...
int main(int argc, char *argv[]){
    ...
    char config_file[20]="HPAF_Config";
  	char buf[8];
    ...
    short configs[16*L], config_num=0;
    int bad_bonds_num[16];
    for (i = 0; i < 16*L; i++)
    {
        configs[i] = -1;
        bad_bonds_num[i] = -1;
    ...
    del1 = 0;
      ...
      {
          for(i=0; i < config_num*L; i++)
            configs[i] = -1;
          config_num = 0;
      }
      else if(Logfnow < 0.125 && CurrentState == MinState && config_num<16)
          Fit_Config(site, configs, &config_num, bad_bonds_num, neighbors, neighloc);
  	}
    ...
    for(j=0; j<config_num; j++)
      printf("%-6d", bad_bonds_num[j]);
  	fp=fopen(dos_file,"w");
    printf("\nState[0] = %-6d, %-12.2f\n", state[0].E, state[0].logg);

  	MinStateLogg=state[0].logg;
  	for(i = 0; i < StatesLength; i++)
    {
      printf("%-3d ", i);
      state[i].logg = state[i].logg - MinStateLogg + 0.69314718;
    ...
    fprintf(fp, " HNNP GS bad bonds numbers:\n");
    for(j=0; j<config_num; j++)
      fprintf(fp, "%-6d", bad_bonds_num[j]);
    fprintf(fp, "\n*********** HNNP, L=%d, Lowest Energy=%d, All configurations *************\n", L, MinState);
    for(j=0; j< config_num; j++)
    {
        if(j==0)
           fprintf(fp, "%-10s", "site #");
