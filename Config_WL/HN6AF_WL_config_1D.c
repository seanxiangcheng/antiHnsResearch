...
int main(int argc, char *argv[]){
  	...
  	char dos_file[24]="H6AFDOS_1D";	// Name of the output file of density of states
    char config_file[24]="H6AF_Config_1D";
  	char buf[8];
    ...
    for(j = 0; j <100; j++) {
  	...
  	printf("\n\n L=%d; Flatness=%f;\n\n", L, Flatness);
  	MaxLength = (7*L + 10); // this is just for HN3
  	int site[L+1];
    int neighbors[7*L+10], neighloc[L+2];
    short configs[32*(L+1)], config_num=0;
    for (i = 0; i < 32*(L+1); i++)
        configs[i] = -1;
    
    Initial_Steps= (double)((double)K*100*(double)L*(double)L);
    ...
    fprintf(fp, "*********** HN6, L=%d, Lowest Energy=%d, All configurations *************\n", L, MinState);
