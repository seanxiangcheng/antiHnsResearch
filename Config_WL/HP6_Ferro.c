...
void commandlineparse(int argc, char **argv,int *k,BondType *k0,BondType *k1,BondType *y){  
  int i;
...
// HNNP AFM: -k 18 -0 -100 -1 -100 -y 0
// HN6 AFM : -k 18 -0 -100 -1 -100 -y 1000
...
void out_run(char *fname,int k,TotalEnergy minenergy){
  ...
  e_dens = (double)minenergy/BONDSCALE/ldexp(1,k);
  fp = fopen(fname,"a");
  fprintf(fp,"%4d  %20.10g  %20.10g\n",k,(double)minenergy/BONDSCALE,e_dens);
  fclose(fp);
}
...
/*   rnd = gsl_rng_alloc(gsl_rng_mt19937); */
  commandlineparse(argc,argv,&maxk,&k0,&k1,&y);
  ...
  for(k=2; k<maxk; k++){
    ...
    /*     } */
    numlines = initHanoi6graph(nv,k0,k1,y);//get HNNP for y=0
    j = 1;
    while( (j *= 2) <= nv){
      //            testgraph(nv+1);
