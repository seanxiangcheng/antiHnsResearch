...
void commandlineparse(int argc, char **argv,int *k,int *maxinst,int *seed,float *f){  
  int i;
  ...
  for (i = 1; i < argc; i++){  //Start at i = 1 to skip the command name.
	...
	break;
      case 's':       *seed = atoi(argv[++i]);
        break;
      case 'f':       *f = 0.01*atoi(argv[++i]);
        break;
      default:
        fprintf(stderr,"\nError:  Incorrect option %s\n",argv[i]);
        fprintf(stderr,"\nAvailable options: \n\
  -k Size of Hanoi Network N=2^k+1 \n\
  -i instances:  set number of instances to run \n\
  -s seed: random seed\n\
  -f 100*fraction of Ferro bonds (use Gaussian for f=2!)\n\
\n");
        exit(0);
      }//switch
    }//if
...
void opennewfile(char *fname,int k,float f,int seed1){
  int i=0;
  FILE *fp;
  ...
  fname[i++] = '6';
  fname[i++] = '_';
  i = inttostring((int)(100*f+.5),i,fname);
  fname[i++] = '.';
  i = inttostring(seed1,i,fname);
  
  fp = fopen(fname,"w");
  fprintf(fp,"#  k      f      seed1  \n");
  fprintf(fp,"#  %-5d  %-5.2f  %d\n\n",k, f, seed1);
  fclose(fp);
}


void out_run(char *fname,int nv,int inst,TotalEnergy energy0,TotalEnergy minenergy){
  FILE *fp;
  
  fp = fopen(fname,"a");
  //fprintf(fp,"%4d %8d %4d %20.10g %20.10g\n",nv,inst,0,(double)minenergy/BONDSCALE,(double)energy0/BONDSCALE);
  fprintf(fp,"%-10d %-6d %-6d %-12.8g %-16.8f %-18.8g\n",nv,inst,0,(double)minenergy/BONDSCALE,(double)minenergy/BONDSCALE/(double)nv,(double)energy0/BONDSCALE);
...
/*
"initbonds" assigns a +1 bond to exactly a fraction f of the edges, and
to a (1-f) fraction -1 bonds. 
*/
int initbonds(int numlines, float f){
  int balance=0,l=0;
  int i,k;
  ...
  for(i=0; i<numlines; i++){
    minus[i] = i;
    cutlist[i] = BONDSCALE;
    bondlist[i] = BONDSCALE;
    balance += BONDSCALE;
  }
  l = numlines;
  while(k-- > 0){
    i = (int)(gsl_rng_uniform(rnd)*l);
    minus[i] = minus[--l];
  }
  for(i=0; i<l; i++){
    cutlist[minus[i]] = -BONDSCALE;
    bondlist[minus[i]] = -BONDSCALE;
  }
  return(balance);
}
...
int main(int argc, char **argv){ 
  int numlines,maxinstance,k,i,j,maxbonds;
  ...
  commandlineparse(argc,argv,&k,&maxinstance,&seed,&f);
  nnv = ldexp(1,k);  /* =2^k !? */     
  maxbonds = (int)(3.2*nnv);
  ...
  opennewfile(outfile,k,f,seed);
  for(inst=0; inst<maxinstance; inst++){
    energy_offset = 0;
    nv = nnv;
    ...
    numlines = initHanoi6graph(nv);
    //    ++nv;                                                                                                                          
    if(f > 1.5){//use Gaussian bonds                                                                                                     
      balance = initGauss(numlines);
    }else{
      balance = initbonds(numlines,f);
    }
    j = 1;
    while( (j *= 2) <= nv){
      //            testgraph(nv+1);                                                                                                     
    ...
    {
        fp = fopen(outfile, "a");
        fprintf(fp,"%-10s %-6s %-6s %-12s %-16s %-18s\n","#Nodes", "Inst", "frac","Low E", "E density", "Energy0");
        fclose(fp);
    }
    out_run(outfile,nv+1,inst,balance,energy_offset);
  }
  free(bondlist);
  free(cutlist);
  free(V);
