...
int * FindNeighbors(n){ 
    ...
    if(n==0)
    {
        ...
        for (i = 0; i < K-1; i++)
        {
            if(Check_Repeat_Neig(neighbors, n))
                neighbors[index++] = Intpow(2, i+1);
        }
...
void Initialize(short sites[], int neighbors[], int neighloc[], double temps[]){
  ...
  CurrentEnergy = neig_index/2;
  CurrentMag = (L+1) * sites[0];
...
int main(int argc, char *argv[]){
  ...
  char energy_file[32] = "H6AF1D_E_k";	// Name of the output file of density of states
  char mag_file[32] = "H6AF1D_m_k";
  char energy_low_file[32] = "H6AF1D_E_low_k";
  char mag_low_file[32] = "H6AF1D_m_low_k";
  ...
  printf("\n    ************ HN6 AF Ising Simulated Annealing Running ************ \n");
  printf("\n      argc: %d\n      ",argc);
  for(j = 0; j < argc; j++){
      printf("%s ", argv[j]);
  }
  ...
  short sites[L+1];
  int neighbors[7*L+8], neighloc[L+2]; 	// store the neighbors index of all sites
  ...
  strcat(energy_file, "_r");
 	strcat(mag_file, "_r");
  strcat(energy_low_file, "_r");
  strcat(mag_low_file, "_r");	
  sprintf(buf,"%d", name);
 	strcat(energy_file, buf);
 	strcat(mag_file, buf);
  strcat(energy_low_file, buf);
  ...
  for (repeat_iter = 0; repeat_iter < repeat_num; repeat_iter++){
    for (iter_now = 0; iter_now < iter_num; iter_now++){
      ...
      if(gsl_rng_uniform(rnd)<(20.0/(double)(repeat_num*iter_num))){
        ...
        //printf("      Check State = %d; check Mag =%d\n", Check_CurrentEnergy(sites, neighbors, neighloc), Check_CurrentMag(sites));
      }
  ...
  for(i = 0; i < T_L * iter_num; i++){
    energy[i] = energy[i] / repeat_num / (L+1); 
    mag[i] = mag[i] / repeat_num / (L+1);
	}
