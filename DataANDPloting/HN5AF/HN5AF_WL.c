...
void Initialize(struct StateInfo state[], int site[], int neighbors[], int neighloc[]){
    ...
    state[0].E = (Intpow(2,K+2)+Intpow(2,K)-8)/2;
    state[0].logg = 0.0;
    state[0].H = 0.0;
    ...
    StatesLength = 1;
  	CurrentState = (Intpow(2,K+2)+Intpow(2,K)-8)/2;
    CurrentIndex = 0;
...
int main(int argc, char *argv[]){
  	...
  	char dos_file[20]="H5AFDOS";	// Name of the output file of density of states
  	char buf[8];
    ...
    Initial_Steps= (double)((double)K*30*(double)L*(double)L);
